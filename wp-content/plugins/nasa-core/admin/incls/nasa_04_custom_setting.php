...
function nasa_options_variation_swatches($of_options = array()) {
    ...
    // More - Less items
    $of_options[] = array(
        "name" => esc_html__('Show Less, More Items', 'nasa-core'),
        "desc" => esc_html__('Show Less, Show More variation items', 'nasa-core'),
...
function nasa_options_woo_open($of_options = array()) {
    ...
    $of_options[] = array(
        "name" => esc_html__("WooCommerce Extended", 'nasa-core'),
...
function nasa_options_share_follow($of_options = array()) {
    ...
    $of_options[] = array(
        ...
        "options" => array(
            ...
            "whatsapp" => esc_html__("WhatsApp", 'nasa-core'),
            // "viber" => esc_html__("Viber", 'nasa-core'),
...
function nasa_options_global_nasa_core($of_options = array()) {
    ...
    $of_options[] = array(
        ...
        "std" => 0,
        "type" => "switch",
        'desc' => esc_html__('Not only is layout responsive, but also the theme uses the device detect technology to switch the web layout compatible on each type of user device', 'nasa-core')
    );
    
    $of_options[] = array(
        "name" => esc_html__("Mobile Layout Mode", 'nasa-core'),
        "id" => "mobile_layout",
        "std" => "df",
        "type" => "select",
        "options" => array(
            "df" => esc_html__("Default", 'nasa-core'),
            "app" => esc_html__("App Simulation", 'nasa-core')
        )
    );
