...
class Vidieu_Dup_Requests_Guard_V2_Ultimate {
    ...
    public function setup_route_optimization() {
        if (is_cart()) {
            ...
            // General optimization for other pages (home, product, post, contact)
            add_action('wp_enqueue_scripts', array($this, 'ultimate_general_cleanup'), 9999);
            add_action('wp_print_scripts', array($this, 'late_general_cleanup'), 1);
            add_action('wp_print_footer_scripts', array($this, 'final_general_cleanup'), 9999);
            $this->log_fix('General optimization registered for non-cart pages');
        }
    }
    ...
    /**
     * ULTIMATE GENERAL CLEANUP - For non-cart pages (home, product, post, contact)
     */
    public function ultimate_general_cleanup() {
        global $wp_scripts, $wp_styles;
        ...
        // Common scripts whitelist for all pages
        $allowed_scripts = array(
            // Core essentials
            'jquery',
            'jquery-core',
        ...
        if (is_front_page() || is_home()) {
            // Home page specific
            $allowed_scripts[] = 'elessi-home';
            $allowed_scripts[] = 'elessi-slider';
        } elseif (is_product()) {
            // Product page specific
            $allowed_scripts[] = 'wc-single-product';
            $allowed_scripts[] = 'wc-add-to-cart-variation';
            $allowed_scripts[] = 'zoom';
            $allowed_scripts[] = 'flexslider';
            $allowed_scripts[] = 'photoswipe';
            $allowed_scripts[] = 'photoswipe-ui-default';
        } elseif (is_single()) {
            // Blog post specific
            $allowed_scripts[] = 'comment-reply';
        } elseif (is_page('contact') || is_page('lien-he')) {
            // Contact page specific
            $allowed_scripts[] = 'google-maps';
        }
        
        // Apply script whitelist
        foreach ($wp_scripts->queue as $key => $handle) {
            if (!in_array($handle, $allowed_scripts)) {
                wp_dequeue_script($handle);
                $this->removed_scripts[$handle] = 'ultimate_general_cleanup';
            }
        }
        
        // Common styles whitelist
        $allowed_styles = array(
            // Core
            'wp-block-library',
            'wc-block-style',
        ...
        // Page-specific style additions
        if (is_product()) {
            $allowed_styles[] = 'photoswipe';
            $allowed_styles[] = 'photoswipe-default-skin';
        }
        
        // Apply style whitelist
        foreach ($wp_styles->queue as $key => $handle) {
            if (!in_array($handle, $allowed_styles)) {
                wp_dequeue_style($handle);
                $this->removed_styles[$handle] = 'ultimate_general_cleanup';
            }
        }
        
        // Record after counts
        $this->scripts_after_whitelist = count($wp_scripts->queue);
        $this->styles_after_whitelist = count($wp_styles->queue);
        
        $this->log_fix(sprintf(
            'General cleanup: Scripts %d→%d, Styles %d→%d',
            $this->scripts_before_whitelist,
            $this->scripts_after_whitelist,
            $this->styles_before_whitelist,
            $this->styles_after_whitelist
        ));
    }
    
    /**
     * Late general cleanup
     */
    public function late_general_cleanup() {
        global $wp_scripts;
        
        $this->current_hook = 'wp_print_scripts';
        $this->current_priority = 1;
        
        // Remove any newly added scripts
        $late_scripts = array(
            'gc-kis-v2', // Kaspersky
            'tp-tools',
            'revmin',
        ...
        foreach ($late_scripts as $handle) {
            if (wp_script_is($handle)) {
                wp_dequeue_script($handle);
                wp_deregister_script($handle);
                $this->removed_scripts[$handle] = 'late_general_cleanup';
            }
        }
    }
     ...
     */
    public function final_general_cleanup() {
        $this->current_hook = 'wp_print_footer_scripts';
        $this->current_priority = 9999;
        
        // Remove any inline scripts that shouldn't be there
        add_filter('script_loader_tag', array($this, 'filter_general_scripts'), 10, 3);
        
        // Log final state
        $this->log_fix('Final general cleanup completed');
    }
    
    /**
     * Filter general scripts
     */
    public function filter_general_scripts($tag, $handle, $src) {
        // Block external tracking/analytics on non-essential pages
        if (!is_checkout() && !is_cart()) {
            ...
            foreach ($blocked_domains as $domain) {
                if (strpos($src, $domain) !== false) {
                    $this->removed_scripts[$handle] = 'blocked_domain:' . $domain;
                    return '<!-- Blocked by VD Ultimate: ' . $handle . ' -->';
                }
            }
        }
