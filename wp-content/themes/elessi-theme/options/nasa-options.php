...
function elessi_get_options() {
    ...
    if (function_exists('nasa_check_is_phone') && nasa_check_is_phone() && (!isset($options['enable_nasa_mobile']) || $options['enable_nasa_mobile'])) {
        ...
        if (isset($options['mobile_layout']) && $options['mobile_layout'] == 'app') {
             ...
             */
            if (isset($options['event-after-add-to-cart']) && $options['event-after-add-to-cart'] === 'popup') {
...
function elessi_enqueue_style() {
    ...
    if (NASA_WOO_ACTIVED) {
        ...
        if (is_product()) {
             ...
             */
            if ($inMobile && isset($nasa_opt['mobile_layout']) && $nasa_opt['mobile_layout'] == 'app') {
                wp_enqueue_style($prefix . '-style-single-product-app', ELESSI_THEME_URI . '/assets/css/style-single-product-app.css', array(), $themeVersion);
            }
        }
        ...
        if (isset($nasa_opt['css_canvas']) && $nasa_opt['css_canvas'] == 'sync') {
            ...
            wp_enqueue_style($prefix . '-style-mobile-menu', ELESSI_THEME_URI . '/assets/css/style-mobile-menu.css', array(), $themeVersion);
        }
...
function elessi_enqueue_scripts() {
    ...
    if (NASA_WOO_ACTIVED) {
        ...
        if (is_product()) {
             ...
             */
            if ($inMobile && isset($nasa_opt['mobile_layout']) && $nasa_opt['mobile_layout'] == 'app') {
                wp_enqueue_script($prefix . '-single-product-mobile-app', ELESSI_THEME_URI . '/assets/js/min/single-product-mobile-app.min.js', array('jquery'), $themeVersion, true);
            }
        }
        ...
        if (isset($nasa_opt['fake_purchase']) && $nasa_opt['fake_purchase']) {
            if (isset($nasa_opt['fake_purchase_ct']) && $nasa_opt['fake_purchase_ct']) {
                ...
                if (!empty($p_data)) {
                    ...
                    if (!empty($array_js)) {
                        wp_enqueue_script($prefix . '-fk-purchased', ELESSI_THEME_URI . '/assets/js/min/nasa-fk-purchased.min.js', array('jquery'), $themeVersion, true);
