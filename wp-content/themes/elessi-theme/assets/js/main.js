...
//    if($('.ns-sub-Account .nasa-sub-account').find('.accordion').length){
//        $('.nasa-sub-account').find('.accordion')[0].remove();
//    }
...
$('body').on('click', '.nasa-menu-accordion .li_accordion > a.accordion', function(e) {
    e.preventDefault();
    var acc = $('.ns-sub-account').find('.li_accordion');
    var ths = $(this).parent();
    var cha = $(ths).parent();
    if (!$(ths).hasClass('active')) {
        ...
        if(acc.hasClass('active')) {
            $(acc).find('>.nav-dropdown-mobile').slideUp(300).parent().removeClass('active');
        }
